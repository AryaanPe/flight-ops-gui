...
def safetyCalculator(city, time):
    ...
    string_stats = cities_weather[cities_weather["City"] == city][time].values[
        0
    ]  # converts string dict to dictionary
...
def update_info():
    ...
    if not fl_df[fl_df["FlightID"] == flight_id].empty:
        ...
        primary_path, primary_time, alternate_path, alternate_time = (
            find_optimized_path(G, dep_city, arr_city, unsafe_nodes)
        )

        label_safety.config(
            text=f"Safety levels at a particular time:\n{get_safety_info(dep_city, dep_time)}"
        )
        ...
        if alternate_path:
            label_alternate_path.config(
                text=f"Rerouted path: {alternate_path} (Total Time: {alternate_time} minutes)"
            )
            label_unsafe_cities.config(
                text=f"Unsafe cities: {', '.join(unsafe_nodes)}", foreground="red"
            )
    ...
    servicingData = pd.DataFrame(
        {
            "Days Since Servicing": [
                fl_df[fl_df["FlightID"] == flightID]["Days_Since_Serving"].values[0]
            ],
            "Warranty Status": [
                fl_df[fl_df["FlightID"] == flightID]["Warranty_Status"].values[0]
            ],
            "Days Since Purchase": [
                fl_df[fl_df["FlightID"] == flightID]["Days_Since_Purchase"].values[0]
            ],
            "Company": [
                modelMapping[fl_df[fl_df["FlightID"] == flightID]["Model"].values[0]]
            ],
        }
    )
