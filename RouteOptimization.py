...
def create_graph(nodes, graph_data):
    ...
    G.add_weighted_edges_from(
        (key[0], key[1], value) for key, value in graph_data.items()
    )
    ...
    if unavailable_nodes:
        for unavailable_node in unavailable_nodes:
            if unavailable_node in primary_path:
                ...
                try:
                    alternate_path = nx.dijkstra_path(
                        temp_G, start, end, weight="weight"
                    )
                    alternate_time = nx.dijkstra_path_length(
                        temp_G, start, end, weight="weight"
                    )
...
def plot(G, path):
    ...
    nx.draw_networkx_nodes(
        G, pos, node_size=700, node_color="skyblue", edgecolors="black", linewidths=1
    )
    nx.draw_networkx_nodes(
        G, pos, nodelist=path, node_color="orange", node_size=700, linewidths=2
    )
